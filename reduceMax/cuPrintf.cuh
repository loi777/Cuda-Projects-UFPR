...
*/
/*
 * NOTE: I DON'T OWN THE ORIGINAL CODE, I JSUT MADE SOME ENHANCEMENTS TO IT!
 */
...
If you only use it for debugging, the following macros are also available:
cuPrintf_D(...): delegates to cuPrintf in debug mode, noop in release mode.
cudaPrintfDisplay_D(...): delegates to cudaPrintfDisplay in debug mode, noop in release mode.
