...
int main(int argc, char **argv) {
  ...
  chronometer_t chrono_reduce;
  chrono_start(&chrono_reduce);
  for (int i = 0; i < nR; ++i) 
    reduceMax_persist<<<NP*THREADS, THREADS>>>(d_max, d_input, numElements);
  cudaDeviceSynchronize();
  chrono_stop(&chrono_reduce);
  ...
  chrono_report_TimeInLoop( &chrono_reduce, (char *)"reduceMax_persist", nR);
  double reduce_time_seconds = (double) chrono_gettotal( &chrono_reduce )/((double)1000*1000*1000);
