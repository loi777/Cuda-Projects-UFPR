////
// wb4.h: Header file for Parallel Progr. with CUDA (UFPR/Dinf)
// version 0.4 by W.M.N.Zola
////
...
struct wbImage_t
{
    // mofified by W.M. N. Zola aug/2016
    // - added pich info (for different strides in image data arrays)
    // - added capacity for images with only one channel
    // - changed *data to point to (unsigned char *) type
    //   (each channel uses one unsigned char)
    ...
    wbImage_t( int imageWidth = 0, int imageHeight = 0, 
               int imageChannels = wbInternal::kImageChannels, 
               int imageColors = wbInternal::kImageColorLimit,
               int imagePitch = 0 /* added */ ) 
      : width(imageWidth), height(imageHeight), 
        channels(imageChannels), colors(imageColors), 
        pitch(imageWidth), data(NULL)        // defaults pitch to be the same as imageWidth
    {
        channels = imageChannels;
        pitch = imagePitch == 0 ? imageWidth : imagePitch;
        const int numElements = pitch * height * channels; // changed width to pitch

        // Prevent zero-length memory allocation
        if (numElements > 0)
            data = new unsigned char[numElements];
    ...
    void print( void )
    {
      std::cout << "[w:" << width << " h:" << height <<  " channs:" << channels 
                << " c:" << colors << " pitch:" << pitch << "]\n" ;
    }
};

// For assignments MP6 & MP11
// WZ for default if pith = 0, use same pitch as the image width
wbImage_t wbImport(const char* fName, int pitch = 0 )
{
    ...
    if (magic != "P6" && magic != "P5")
    {
        std::cerr << "Error reading image file " << fName << ". " 
                  << "Expecting 'P6' or 'P5' image format but got '" 
                  << magic << "'" << std::endl;
        inFile.close();
        std::exit(EXIT_FAILURE);
    }
    ...
    inFile >> image.width; // using defaults (WZola) (0 by 0 image, no data)
    ...
    // wz correct the image default for pitch
    if( pitch == 0 )
        image.pitch = image.width;
    else
        image.pitch = pitch;
    
    if( magic == "P5" ) { // wz to correct image channels
        image.channels = 1;
    }

    const int numElements = image.pitch * image.height * image.channels; // WZ changed width for pitch
    ...
    {
        std::cerr << "Size of image in file " << fName 
        << " does not match its header. Expecting " << numElements 
        << " bytes, but got " << elementsRead << std::endl;
        delete [] rawData;
        std::exit(EXIT_FAILURE);
    }
...
void wbExport(const char* fName, wbImage_t image )
{
    ...
    if( image.channels == 3 ) 
        ...
        std::cerr << "Error writing image file " << fName << ". " 
                  << "Don\'t know how to write an image with " << image.channels 
                  << "channels" << std::endl;
        outFile.close();
        std::exit(EXIT_FAILURE);
    }
    ...
    outFile << "# written by wblib\n" ;
    outFile << image.width << " "; // using defaults (WZola) (0 by 0 image, no data)

    outFile << image.height << "\n";

    if (outFile.fail()) {
        std::cerr << "Error writing height of image in file " << fName << std::endl;
        outFile.close();
        std::exit(EXIT_FAILURE);
    }
    ...
    if (outFile.fail() || image.colors != wbInternal::kImageColorLimit)
    {
        std::cerr << "Error writing colors value of image in file " << fName << std::endl;
        outFile.close();
        std::exit(EXIT_FAILURE);
    }


    const int numElements = image.width * image.height * image.channels; // WZ 
    std::cout << "writing " << numElements << " image bytes to file\n";

    outFile.write(reinterpret_cast<char*>(image.data), numElements);

//    const int elementsWritten = static_cast<int>(outFile.gcount());

    outFile.close();

/*    if (elementsWritten != numElements)
    {
        std::cerr << "Size of image in file written " << fName 
        << " does not match its header. Expected writing " << numElements 
        << " bytes, but wrote only " << elementsWritten << std::endl;
        std::exit(EXIT_FAILURE);
    }
*/
...
int wbImage_getPitch(const wbImage_t& image)     // added by wz
{
    return image.pitch;
}
...
unsigned char* wbImage_getData(const wbImage_t& image)
{
     return image.data;
}
...
// WZ added
wbImage_t wbImage_new(const int imageWidth, const int imageHeight, const int imageChannels, 
                      const int imagePitch )
{
    wbImage_t image( imageWidth, imageHeight, imageChannels, imageChannels );
    return image;
}
...
void wbSolution(const wbArg_t& args, const T& t, const S& s, const U& u)
{
    ...
    {
        std::cout << "Size of the matrix in solution file " << wbArg_getInputFile(args, args.argc - 2) 
                  << " does not match. ";
        std::cout << "Expecting " << solnRows << " x " 
                  << solnColumns << " but got " << s << " x " << u << ".\n";
    }
    else // Check solution
    {
        int errCnt = 0;
        ...
        for (int row = 0; row < solnRows; ++row)
        {
            for (int col = 0; col < solnColumns; ++col)
            {
                ...
                if (!wbInternal::wbFPCloseEnough(expected, result))
                {
                    if (errCnt < wbInternal::kErrorReportLimit)
                    {
                        std::cout << "The solution did not match the expected results at column " 
                                  << col << " and row " << row << "). ";
...
void wbSolution(const wbArg_t& args, const wbImage_t& image)
{
    ...
    std::cout << "checking against image " << wbArg_getInputFile(args, args.argc - 2) << "\n";

    if (solnImage.width != image.width || solnImage.height != image.height)
    {
        std::cout << "Size of the image in file " << wbArg_getInputFile(args, args.argc - 2) 
                  << " does not match. ";
        std::cout << "Expecting " << image.width << " x " << image.height 
                  << " but got " << solnImage.width << " x " << solnImage.height << ".\n";
    }
    else // Check solution
    {
        //WZ wbInternal::wbImage_save(image, args, "transformed_image.ppm");
        ...
        for (int i = 0; i < image.width; ++i)
        {
            for (int j = 0; j < image.height; ++j)
            {
                ...
                {
                    //WZ const int index = (j * image.width + i) * image.channels + k;
                    //WZ const float error = fabs(solnImage.data[index] - image.data[index]);
                    // WZ ... changed width by pitch
                    const int index = (j * image.pitch + i) * image.channels + k;
                    const int error = abs( (int)solnImage.data[index] - (int)image.data[index] );

                    // if (error > (1.0f / wbInternal::kImageColorLimit * tolerance))
                    if (error > 7) // WZ small tolerance, let's see ...
                    {
                        if (errCnt < wbInternal::kErrorReportLimit)
                            std::cout << "Image pixels do not match at position (" 
                                      << j << ", " << i << ", " << k << "). got:" 
                                      << (int)image.data[index] << " exp:" << (int)solnImage.data[index] << "\n";
